...
const COLOR: Record<
  string,
  { card: string; label: string; sub: string; icon: string; hint: string }
> = {
  "arm64-v8a": {
    card: "bg-blue-500/5 border border-blue-500/20 hover:border-blue-500/40 hover:bg-blue-500/10 shadow-[0_0_15px_rgba(59,130,246,0.1)] variant-card-blue",
    label: "text-blue-300 variant-label-blue",
    sub: "text-blue-400/60 variant-sub-blue",
    icon: "text-blue-400 variant-icon-blue",
    hint: "Android 5.0+",
  },
  "armeabi-v7a": {
    card: "bg-violet-500/5 border border-violet-500/20 hover:border-violet-500/40 hover:bg-violet-500/10 shadow-[0_0_15px_rgba(168,85,247,0.1)] variant-card-violet",
    label: "text-violet-300 variant-label-violet",
    sub: "text-violet-400/60 variant-sub-violet",
    icon: "text-violet-400 variant-icon-violet",
    hint: "Android 10 – 12",
  },
  x86_64: {
    card: "bg-amber-500/5 border border-amber-500/20 hover:border-amber-500/40 hover:bg-amber-500/10 shadow-[0_0_15px_rgba(245,158,11,0.1)] variant-card-amber",
    label: "text-amber-300 variant-label-amber",
    sub: "text-amber-400/60 variant-sub-amber",
    icon: "text-amber-400 variant-icon-amber",
    hint: "Android 12 and Up",
  },
  x86: {
    card: "bg-emerald-500/5 border border-emerald-500/20 hover:border-emerald-500/40 hover:bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.1)] variant-card-emerald",
    label: "text-emerald-300 variant-label-emerald",
    sub: "text-emerald-400/60 variant-sub-emerald",
    icon: "text-emerald-400 variant-icon-emerald",
    hint: "Android 5.0+ (32-bit x86)",
  },
  universal: {
    card: "bg-slate-900/45 border border-white/5 hover:border-indigo-500/30 hover:bg-slate-900/60 shadow-md variant-card-zinc",
    label: "text-slate-200 variant-label-zinc",
    sub: "text-slate-400 variant-sub-zinc",
    icon: "text-slate-400 variant-icon-zinc",
    hint: "All architectures",
  },
};
...
export default function ProjectDetail({
  ...
  const data = (releasesData as Record<string, ProjectData>)[projectKey];

  if (!data || !data.versions || data.versions.length === 0) {
    return (
      <div className="min-h-screen text-slate-200 py-6 flex items-center justify-center">
        <div className="text-center space-y-4">
          <h1 className="text-2xl font-bold text-red-400">Project Not Found</h1>
          <p className="text-slate-400">
            No release details found for key: {projectKey}
          </p>
          <Link
            href="/projects"
                ...
                >
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      ...
                      {abi === "arm64-v8a" && (
                        <span className="recommended-badge inline-flex items-center px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wider rounded-full shadow-[0_0_10px_rgba(59,130,246,0.15)] animate-pulse shrink-0">
                          Recommended
                        </span>
                      )}
                    </div>
                    <div className={`text-xs font-mono ${c.sub}`}>
                      {c.hint} &bull; {size || "Unknown size"}
                    </div>
                  </div>
                  <span
                    className={`group-hover:translate-y-0.5 transition-transform duration-300 ${c.icon}`}
                  >
                    <DownloadIcon />
                  </span>
                </a>
                    ...
                    <summary className="flex items-center justify-between px-6 sm:px-8 py-4 cursor-pointer hover:bg-white/5 transition-colors list-none">
                      <div className="space-y-0.5">
                        ...
                        <div className="text-xs font-mono text-slate-500">
                          {v.releases.length} APK variant
                          {v.releases.length !== 1 ? "s" : ""}
                        </div>
                      </div>
                      <svg
